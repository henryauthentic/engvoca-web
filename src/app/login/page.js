...
import { Mail, Lock, Eye, EyeOff, BookOpen, Loader2, AlertCircle } from "lucide-react";
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-primary-500 via-primary-600 to-secondary-700 p-4">
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="relative w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-white/60 text-sm mt-2">Đăng nhập để tiếp tục học</p>
        </div>
        ...
        <div className="bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl p-8">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <button type="submit" disabled={loading}
              ...
              {loading ? <Loader2 className="w-5 h-5 mx-auto animate-spin" /> : "Đăng nhập"}
            </button>
          </form>
          ...
          <p className="text-center text-sm text-gray-500 mt-6">
            Chưa có tài khoản?{" "}
            <Link href="/register" className="text-primary-600 font-semibold hover:text-primary-700">Đăng ký ngay</Link>
          </p>
        </div>
      </motion.div>
    </div>
